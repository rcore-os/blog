...
## 六、对于向老师所说的中断处理过程中的同步异步分别如何处理的问题的回答
首先其实无论时同步还是异步,他们的底层都是一样的中断处理函数,都是一样设置状态标记（`IDMAC_DONE_FLAG`/`IDMAC_ERROR_FLAG`），然后唤醒等待的任务/协程
只是在硬件读写处理前，我们必须使用**互斥锁（Mutex）**来进行并发保护，且唤醒后在系统调度上存在核心差异：

### 1. 硬件互斥锁定与防死锁设计
由于 SDMMC 硬件是单通道独占设备，我们在发起 DMA 读写前必须获取驱动锁：
* **同步端**：在获取互斥锁（如 `SpinNoIrq` 或操作系统互斥量）后，如果等待中断，会挂起当前线程。此时其他尝试获取锁的线程会被排队，直至前一个线程在中断中被唤醒并释放锁。
* **异步端**：为了防止协程挂起时导致底层线程“死锁”，我们不能在 `.await` 期间持有物理自旋锁。上层对接时使用**异步互斥锁（Async Mutex）**，当一个协程挂起时，它会主动把锁让出来，允许同一物理线程内的其他协程正常运行。

### 2. 中断唤醒的多态分发（Waker 多态）
在中断触发并调用 `notify_one` 时，底层通过 Rust 标准库 `Waker` 的虚函数表实现多态性，自动完成同步与异步的区分唤醒：
* **同步的 Waker**：对应 `AxWaker`。其 `wake()` 实现会通知操作系统的调度器，直接将对应的**物理线程**状态改为 `Runnable`，从而在中断返回后可以通过内核调度器触发抢占式切换，物理线程恢复运行并拷贝数据。
* **异步的 Waker**：对应协程执行器的 Waker。其 `wake()` 实现仅仅是将**协程状态机**标为 Ready 放入执行器的就绪链表。中断返回后物理线程不阻塞，由执行器随后 poll 推进协程状态。
...
* [训练营六周周报](https://github.com/xianxw/Asyncos/tree/main/%E6%8A%A5%E5%91%8A)
* [训练营其他阶段的工作](https://github.com/xianxw/Asyncos/tree/main)