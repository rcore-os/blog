...
pub fn emm(a: i32, b: i32) -> f64 {
    ...
    a as f64 + b as f64    // 或写为：`return a as f64 + b as f64;` 或 `return f64::from(a) + f64::from(b);`
}
```
...
```rust
pub fn emm<T>(op: T) -> (i32, &'static str, i32) {  // op会在编译时自动根据所调用时的类型生成对应类型标签的多态方法
    op;
    (1, "heke", 1228)
}
```
...
enum EnumTmp {
    A, B, C, D = 233,
    E, StructEMM { attr1: i32, attr2: i32 }, Opt(Option<u8>),
}
fn random_choose_a_state() -> EnumTmp { todo!() }
// 因为枚举一用全用，所以当enum前声明权限pub的时候，所有的字段都是pub，这不同于struct
fn main() {
    use EnumTmp::*;                     // 引入变体，下面match就能少写EnumTmp::前缀
    let state = random_choose_a_state();
    match state {
        A => println!("匹配到变体 A，每个分支用逗号分割！"),  // 匹配一个具体的变体
        Opt(Some(233)) => println!("Opt 里恰好装着 233"),    // 还能往里钻一层，匹配「数字」字面量！
        Opt(Some(n)) => println!("Opt 里装着数字 {}", n),    // 也能把里面的值「绑定」成变量 n 拿出来用
        StructEMM {attr1: x, attr2} => {	// 解构结构体变体：attr1 取别名 x，attr2 是 attr2: attr2 的缩写
            println!("匹配到结构体变体，attr1是{}，attr2是{}", x, attr2);
        },
        _ => println!("下划线兜底，匹配剩余的所有情况！"),
    }
}
```
...
```rust
impl Clone for StructEmm {
    fn clone(&self) -> Self { ... }	//	为结构体StructEmm实现Clone Trait
}
...
​	进程/线程/协程也各演示一下。多进程，`Command` 拉起子进程去跑别的程序：
...
​	多线程，同一进程里开几个执行流，`channel` 在线程间递话（懒得手动开就上 `rayon` 线程池）：
...
​	协程（异步任务）最轻，`tokio::spawn` 撒一把出去、`join_all` 一起收（运行时用 [Tokio](https://tokio-zh.github.io/)）：
...
​	过程宏，得单独开个 `proc-macro` crate，在 `TokenStream` 语法树上动刀：
```rust
// 过程宏的导出需要专门的crate导出：
/// Cargo.toml：
/// [dependencies]
...
​	TCP，`TcpListener` 绑端口、`incoming()` 一个个收连接：
...
​	发 HTTP 请求最省事是 `reqwest`（同步走 `blocking`、异步直接 `.await`）：
...
常见的Web空间目前[2024.09]有：[Rocket](https://rocket.rs/)，[Actix](https://actix.rs/)，[Axum](https://github.com/tokio-rs/axum)，[Poem](https://github.com/poem-web/poem)，[Salvo](https://salvo.rs/)等等；可以先入为主尝试`Rocket`和`Poem`！Tokio玩家可以入手`Axum`。
...
​	光会语法还不够，Rust 真正能打靠的是 [crates.io](crates.io) 上一堆现成轮子。挑些常用的归归类（更多直接逛官网 / 官文）：

**GUI / 图形 / 游戏**

- [egui](https://github.com/emilk/egui)——简易响应式（immediate mode）GUI；[eframe](https://github.com/emilk/egui/tree/main/crates/eframe) 是它 + GPU 的跨端封装框架
- [Bevy](https://bevy.org)——ECS 架构的游戏引擎
- [winit](https://github.com/rust-windowing/winit)——跨平台桌面窗口创建
- [wgpu](https://wgpu.rs/)——WebGPU 标准实现，一套代码跨端图形 / 计算
- [plotters](https://github.com/plotters-rs/plotters) / [plotlib](https://github.com/milliams/plotlib)——科学绘图

**异步 / 并发**

- [tokio](https://tokio.rs/)——主流通用异步运行时（工作窃取调度）；[smol](https://github.com/smol-rs/smol)——~1500 行的轻量级异步
- [rayon](https://docs.rs/rayon)——一行把串行迭代器变并行（`.iter()` → `.par_iter()`）
- [reqwest](https://docs.rs/reqwest)——异步 / 阻塞 HTTP 请求

**Web 框架**

- [Rocket](https://rocket.rs/)、[Axum](https://github.com/tokio-rs/axum)、[Actix](https://actix.rs/)、[Poem](https://github.com/poem-web/poem)、[Salvo](https://salvo.rs/)——上面 §网络编程 演示的就是 Rocket

**宏 / FFI / WASM**

- [syn](https://docs.rs/syn) + [quote](https://docs.rs/quote)——过程宏里解析 / 生成 AST 的黄金搭档
- `bindgen`（C 头 → Rust）/ `cbindgen`（Rust → C 头）——自动生成 FFI 绑定
- [wasm-pack](https://rustwasm.github.io/wasm-pack/) + `wasm-bindgen`——编 wasm + 生成 JS 胶水

**嵌入式 / 驱动**

- [svd2rust](https://github.com/rust-embedded/svd2rust)——把芯片 SVD 寄存器描述生成 PAC 外设访问库
- `bitflags` / `bytemuck` / `tock-registers` / `bounded_registers` / `register`——位标志 / 安全字节转换 / 寄存器位域
- [ostool](https://crates.io/crates/ostool)——驱动构建 / 配置 / 运行工具；[bare-test](https://crates.io/crates/bare-test)——裸机单元测试框架

**工程化 / 代码质量**

```shell
cargo clippy --all-features --all-targets              # 静态 lint，抓坏味道
cargo fmt --check                                      # 格式化（只检查不改）
cargo tarpaulin --all-features --workspace --out xml   # 测试覆盖率（cargo install cargo-tarpaulin）
